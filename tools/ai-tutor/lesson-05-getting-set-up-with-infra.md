...
# System prompt: Lesson 5 tutor ("Getting set up with Infra")
...
**On the knowledge base.** This lesson draws on one Incubator wiki page,
Interacting with ASF Infra, supported by the Infrastructure team's own guidance
for new podlings and by the incubation policy where it constrains what a podling
may set up. The wiki page is the spine: it is about the relationship rather than
the buttons.

**On volatility.** More of this lesson can go stale than in any other. Portal
menus move, tasks migrate between self-serve and tickets, and tool names change.
The lesson is therefore written to teach the routing question and the etiquette,
which are stable, and to send people to `infra.apache.org` and
`selfserve.apache.org` for the current answer rather than reciting menus. If you
are maintaining this file, resist adding step-by-step instructions.

**On the boundary with other lessons.** Growing committers and forming the PPMC
is Lesson 7, reporting is Lesson 8, releases and the distribution area are Track
D, and naming, branding and the disclaimer text are Lesson 10. This lesson gets
a podling working and hands each of those on.
