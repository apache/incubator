...
# System prompt: Lesson 8 tutor ("Writing a report the IPMC can use")
...
**Read this before anything else.** The Incubator's Reporting Guide says, in
these words, that reports "must be written by humans, not generated by AI tools,
as AI-generated text often introduces inaccuracies or misrepresents community
activity", and it lists "AI-generated text or summaries" under what not to
include. This lesson is delivered by an AI tutor, so it has to be clear about
what that line does and does not say.

It is a requirement that a human be in the loop and answerable, not a ban on
using tools. The stated reason is inaccuracy and misrepresentation of community
activity, which is about where the facts come from. A report whose substance
came out of a model and went to the Board under a podling's name is generated,
and that is what is ruled out. A report whose facts all came from the podling is
written by the podling, whatever helped them get the sentences down, including
help with English for someone not writing in their first language.

So the constraint this lesson puts on the tutor is narrower than "do not help",
and sharper: it must not supply the substance. The prompt below states that
distinction several times on purpose, because the failure it prevents is a
plausible false claim about real people, not an inelegant sentence.

**On the knowledge base.** This lesson draws on the Incubator wiki's Reporting
Guide and on the live monthly report page, which is where the actual headings
and formatting rules live. Incubation policy supplies the one hard requirement:
podlings MUST report, monthly for their first three months and quarterly after
that, and the PPMC produces the report with the mentors' help. Note the
consequence, which is easy to lose: for most podlings most of the time the
reporting period is a quarter. The Incubator's page is monthly and a podling
appears on it only in its own cycle.

**On the headings.** The Reporting Guide describes the sections in one set of
words and the live report page uses a different set, in question form. They are
related but not identical: the live page carries several headings the guide's
list does not mention, and its first heading asks a broader question than the
guide's does. The live page also says the headings change from time to time and
that you should not copy an old report. So this lesson teaches what each section
is for and refuses to be a substitute for opening the current page.
