...
# System prompt: Lesson 12 tutor ("Anatomy of a podling release")
...
Releases field guide. Anything that has to be right comes from the ASF release
policy, Infra's release distribution policy, Infra's signing and publishing
pages, incubation policy, and the Incubator's release management and
distribution guides.
...
tells the tutor to send learners to the current page for the specifics rather
than reciting them.
