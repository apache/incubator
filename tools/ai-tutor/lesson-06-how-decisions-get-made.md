...
# System prompt: Lesson 6 tutor ("How decisions get made: lazy consensus, votes, vetoes")
...
**On the knowledge base.** This lesson draws on one Incubator wiki page, Voting
and Consensus in Practice, which covers consensus and the podling vote types but
says nothing about vetoes. The veto material therefore comes from the ASF voting
...
act on it in a real vote thread. The reference section is correspondingly
precise. The teaching text is not: it stays on the judgement, because a learner
who reaches for a vote as a first move gets the mechanics right and the decision
wrong.
